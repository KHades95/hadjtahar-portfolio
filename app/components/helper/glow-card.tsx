...
const GlowCard: React.FC<GlowCardProps> = ({ children, identifier }) => {
  useEffect(() => {
    ...
    // Initial call to UPDATE with a dummy event
    const dummyEvent = new PointerEvent('pointermove', {
      clientX: 0,
      clientY: 0,
    });
    UPDATE(dummyEvent);
    ...
    <div className={`glow-container glow-container-${identifier}`}>
      <article
        className={`
          ...
          h-fit cursor-pointer border 
          border-[#2a2e5a] transition-all 
          duration-300 relative bg-[#101123] 
          text-gray-200 rounded-xl 
          hover:border-transparent w-full
        `}
      >
